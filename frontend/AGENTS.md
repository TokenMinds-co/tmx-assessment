...
- Every screen now renders real API data. Nothing invented is left in the app, and there is no "Sample data" badge ([docs/dashboard.md](docs/dashboard.md)).
...
- **Helpers, types and constants** sit next to the components that use them, like `components/shared/nav-items.ts`. When those components are in different folders, put the file in `lib/`, like `lib/validation.ts` and `lib/format.ts`. Code in `components/` and `lib/` never imports from `app/`.
