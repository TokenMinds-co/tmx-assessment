...
- **Shared UI built on top** is in [components/shared/](../components/shared/): the TMX Assessment logo, the app shell, the sign-in card, and the assessment pieces such as the test runner, the status badges and the score summary. Where components go is in [project-structure.md](project-structure.md).
...
- Name the app **TMX Assessment** and update the logo to match.
...
- **TMX Assessment lockup** ([components/shared/tmx-assessment-logo.tsx](../components/shared/tmx-assessment-logo.tsx)), used on the sign-in pages: the TMX wordmark artwork, then "Assessment" set in Inter at the same cap height, in the logo's ink color. The wordmark ([public/brand/tmx-wordmark.png](../public/brand/tmx-wordmark.png)) is cropped from the brand's existing logo artwork, so only "Assessment" is type.
- **Mark** ([public/brand/tmx-mark.png](../public/brand/tmx-mark.png)): the white TMX tile, shown at 36px in the sidebar next to "TMX Assessment".
...
- **Topbar** ([components/shared/topbar.tsx](../components/shared/topbar.tsx)): 77px tall (60px on phones), white, with a breadcrumb ("TMX Assessment › page") and the account menu.
...
Two differences from the reference shell: the whole page scrolls (the rail is fixed and the topbar sticky) instead of only `<main>`, and the rail doesn't force itself collapsed below 1280px. The reference did that to fit its editor's chat column, which TMX Assessment doesn't have.

### Candidate pages

- **No app shell.** The `(candidate)` layout is just the page background, and each page draws its own header.
- **The company wordmark** ([candidate-brand.tsx](../components/shared/candidate-brand.tsx)), not the TMX Assessment lockup, because candidates know the company they applied to, not the internal app. The name beside it comes from `COMPANY_NAME` in [lib/brand.ts](../lib/brand.ts), which reads `NEXT_PUBLIC_COMPANY_NAME` and falls back to `TMX Assessment`; it is the wordmark's alt text and the candidate pages' titles. The artwork itself is a file — replace [public/brand/tmx-wordmark.png](../public/brand/tmx-wordmark.png) to match the name you set. See [configuration.md](configuration.md).
...
| Product name | TMX Assessment | | Requested |
| Logo | The TMX wordmark artwork, then "Assessment" set in Inter | Keeps the brand's own artwork, and there's no vector source to redraw it from | Build default |
| Lockup size | 26px by default, with the wordmark and the word tied to one font size | "Assessment" is long: at the wordmark's own 34px the lockup is wider than a phone screen. One size keeps the cap heights matched at any size a caller picks. | Build default |
...
| Candidate pages' brand | The company wordmark, not the TMX Assessment lockup | Candidates know the company they applied to, not the internal app. | Build default |
| The company name candidates see | `NEXT_PUBLIC_COMPANY_NAME`, read through [lib/brand.ts](../lib/brand.ts) and defaulting to `TMX Assessment` | It was hardcoded, so a fork would greet candidates under someone else's name. The wordmark is still an image to replace, but everything set in type follows the variable. Keep it the same as the backend's `COMPANY_NAME`, which names the company in candidates' emails. | Requested |
...
- A vector (SVG) TMX Assessment logo, once the brand's source files are available.
